...
#if defined(NDEBUG)
# error "volumecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6be8dc6af353e808cc0e04e078faf9f77553c9197734a2f6b5523694daa6d04e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // volumecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "volumecoin Signed Message:\n";
...
{
    // volumecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    else if(nHeight < 5200)  
    {
        nSubsidy = 2048 * COIN;
   }

    // Subsidy is cut in half every 5000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 5000); // volumecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // volumecoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // volumecoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // volumecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // volumecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Time for some Volume";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("047154710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1434901717;
        ...
        assert(block.hashMerkleRoot == uint256("0x9214e98800b5add3fb37a3d436901d8b0f64d91c6598eccdfa4dee5eeee6032a"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xdc, 0xe4, 0xb3, 0xdb }; // volumecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("volumecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("volumecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static volumecoinMiner(CWallet *pwallet)
{
    printf("volumecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("volumecoin-miner");
    ...
    try { loop {
        ...
        printf("Running volumecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("volumecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&volumecoinMiner, pwallet));
}
